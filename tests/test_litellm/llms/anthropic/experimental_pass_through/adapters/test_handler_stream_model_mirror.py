...
# Anchor sys.path to this file's location so resolution works regardless of
...
async def test_streaming_message_start_mirrors_requested_alias():
    ...
    with patch("litellm.acompletion", return_value=_EmptyAsyncStream()):
...
async def test_streaming_message_start_without_proxy_metadata_unchanged():
    """SDK-style call (no proxy metadata): behavior is unchanged — the model
    passed to the handler is reported as-is."""
    with patch("litellm.acompletion", return_value=_EmptyAsyncStream()):
        transformed_stream = await (
            LiteLLMMessagesToCompletionTransformationHandler.async_anthropic_messages_handler(
                max_tokens=16,
