...
def test_translate_anthropic_to_openai_demotes_midturn_system_when_enabled(monkeypatch):
    """
    With LITELLM_DEMOTE_MIDTURN_SYSTEM=true, in-sequence system rows are rewritten as user
    rows so chat templates that reject non-leading system messages (e.g. Qwen3 on vLLM)
    accept the request. The hoisted top-level prompt at index 0 keeps `role: "system"`.
    """
...
@pytest.mark.parametrize("env_value", ["", "false", "1", "TRUE_"])
